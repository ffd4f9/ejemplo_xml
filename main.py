...
"""
xml = MD.parse(ruta)
        nodoPacientes = xml.getElementsByTagName("pacientes")[0]
        pacientesxml = nodoPacientes.getElementsByTagName("paciente")
        for pacientexml in pacientesxml:
            datospersonales = pacientexml.getElementsByTagName("datospersonales")[0]
            nodoNombre = datospersonales.getElementsByTagName("nombre")[0]
            nodoEdad = datospersonales.getElementsByTagName("edad")[0]
            nombre = nodoNombre.firstChild.data #nombre
            edad = nodoEdad.firstChild.data #edad
            nodoPeriodos = pacientexml.getElementsByTagName("periodos")[0]
            periodos = int(nodoPeriodos.firstChild.data) #periodos


contador = 0
            while pacientes.elemento(contador)!=None:
                pacientes.elemento(contador).evaluar(True)
                contador+=1
            #aqui escribo el archivo

            nodoPacientes = ET.Element("pacientes")
            for i in range(pacientes.longitud()):
                paciente = ET.SubElement(nodoPacientes,"paciente")
                datosPersonales = ET.SubElement(paciente,"datospersonales")
                ET.SubElement(datosPersonales,"nombre").text=pacientes.elemento(i).getNombre()
                ET.SubElement(datosPersonales,"edad").text=str(pacientes.elemento(i).getEdad())
                ET.SubElement(paciente,"periodos").text=str(pacientes.elemento(i).getPeriodos())
                ET.SubElement(paciente,"m").text=str(pacientes.elemento(i).getM())
                ET.SubElement(paciente,"resultado").text=pacientes.elemento(i).getEstado()
                if pacientes.elemento(i).getN()!=None:
                    ET.SubElement(paciente,"n").text=str(pacientes.elemento(i).getN())
                    if pacientes.elemento(i).getN1()!=None:
                        ET.SubElement(paciente,"n1").text=str(pacientes.elemento(i).getN1())
            ruta = buscar("REPORTES","PACIENTES",".xml")
            archivo = ET.ElementTree(nodoPacientes)
            archivo.write(ruta,xml_declaration=True,encoding='utf-8')
            webbrowser.open(ruta)
            contador = 0
            while pacientes.elemento(contador)!=None:
                pacientes.elemento(contador).desevaluar()
                contador+=1
            ...
            #antes de evaluar se deberia poder crear el pdf
            def buscar(carpeta:str,tipoReporte:str,extension:str):
        i = 1   
        dir = str(__file__).replace("\\","/").replace("main.py","")+carpeta+"/"+tipoReporte
        miFile = Path(dir+str(i)+extension).is_file()
        while miFile !=False:
            i+=1
            miFile = Path(dir+str(i)+extension).is_file()
        return dir+str(i)+extension #ahora ya se cual es el ultimo reporte creado, ahora puedo crear el nuevo
